...
若当前分支是 `main` 或 `master`，先自动创建并切换到功能分支，再执行后续提交。

分支命名：

```text
feat/<YYYYMMDD>-<short-topic>
```

`short-topic` 基于本次改动生成英文短语（kebab-case），如 `commit-skill`。
...
分批策略（按优先级）：
1. 按业务模块分组（如 `api`、`ui`、`hooks`、`utils`）
...
- 默认不自动推送。
- 仅在用户明确要求（如“顺便 push”）时执行 `git push`。
- 若是新建分支，使用 `git push -u origin <branch>`。
